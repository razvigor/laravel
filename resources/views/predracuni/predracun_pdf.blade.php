                    ...
                    <table style="width: 100%;">
                        ...
                        <tr>
                            ...
                            <td style="text-align: left; width: 40%;">
                                ...
                                {{$podatak->kupac_id=='0'?$podatak->kupac_ime : $detalji_kupca->naziv}}
                            </td>
                            <td  style="text-align: left; width: 20%;">
                                <strong> Datum:  </strong>
                            </td>
            ...
            <div class="row">
                <div class="col-xs-6">
                    ...
                    <h4  style="text-align: center; font-size: 20px;">
                        @if($podatak->racun_nota=='N')N O T A -   @endif @if($podatak->status==0)P O N U D A @endif @if($podatak->status==1)R A Č U N @endif br. {{$podatak->broj}} <br>
            ...
            <table style="width: 100%;">
                <thead style="background: #c9dbec;">
                    <tr>
                        ...
                        {{--@if($podatak->status==0)--}}
                        @if($tip=='PR_D' || $podatak->status==0) 
                            <th class="text-center" style="padding: 5px;">Rabat</th>
                        @endif

                        <th class="text-center" style="padding: 5px;">PDV</th>
                ...
                <tbody>
                    ...
                    {{-- KOMPLET REMONT --}}
                    @if($podatak->komplet_remont=='1' && $tip=='PR') 
                    <tr width="100%">                 
                        <td class="text-center">{{$rb=1}}</td>
                        <td class="text-left" style="padding: 5px;"><div>{{$podatak->oem}}</div></td>
                        
						<td class="text-left" style="padding: 5px;">{{$podatak->usluga_naziv}}</td>
                        ...
                        --}}
                        @if($tip=='PR_D') 
                        <td class="text-center">-</td> {{-- rabata nema --}}
                        @endif
                    ...
                    @endif
                        ...
                        @if(($podatak->komplet_remont=='2' && $podatak->status==0) || $tip=='PR_D')
                        ...
                        <tr width="100%" >                 
                            ...
                            <td class="text-center">-</td>{{-- kolicine nema --}}
                            <td class="text-center">-</td>{{-- jed. mjere nema --}}
                        ...
                        {{-- prikaži samo ako je servis    i      račun          i   bez detalja --}}
                        @if($podatak->komplet_remont=='2' && $podatak->status==1 && $tip=='PR')
                        <tr width="100%" >                 
                            <td class="text-center">{{$rb=1}}</td>
                        ...
                        {{-- DIJELOVI --}}
                        {{-- prikaži samo ako je:
                                         predračun i servis turbine                 ili kliknuto dugme detalji--}}
                        @if(($podatak->status==0  && $podatak->komplet_remont=='2') || ($tip=='PR_D'  && $podatak->komplet_remont=='2')) 
                                ...
                                @foreach ($dijelovi as $dio)
                                    @if($tip->oznaka==$dio->tip_oznaka && $dio->komada>0)
                                        <tr width="100%">
                                            ...
                                            <td class="text-center">{{number_format((float)$dio->rabat + $dio->rabat_dio, 2, '.', '')}}%</td>
                                            
                                            <td class="text-center">17 %</td>
                                            <td class="text-right" style="padding: 5px;">{{number_format((float)$dio->komada*$dio->cijena*(100-$dio->rabat - $dio->rabat_dio)/10000, 2, '.', '')}}</td>
                        ...
                        <tr>
                            ...
                            <th @if($tip=='PR') colspan="3" @else colspan="5" @endif></th>
                            <th @if($tip=='PR') colspan="2" @else colspan="3" @endif class="text-left" style="padding: 5px;">Ukupno bez PDV</th>
                            <th class="text-right" style="padding: 5px;">{{number_format((float)$podatak->ukupna_cijena/117, 2, '.', '')}}</th>
                        </tr>                        
                        <tr>
                            <th @if($tip=='PR') colspan="3" @else colspan="5" @endif></th>
                            <th @if($tip=='PR') colspan="2" @else colspan="3" @endif class="text-left" style="padding: 5px;">PDV</th>
                            <th class="text-right" style="padding: 5px;">{{number_format((float)$podatak->ukupna_cijena*(1-1/1.17)/100, 2, '.', '')}}</th>
                        </tr>                        
                        <tr>
                            <th @if($tip=='PR') colspan="3" @else colspan="5" @endif></th>
                            <th @if($tip=='PR') colspan="2" @else colspan="3" @endif class="text-left" style="padding: 5px; background: #c9dbec;">Ukupno sa PDV</th>
