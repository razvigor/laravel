          ...
          <form method="post" enctype="multipart/form-data" action="{{ route('user.update', $user) }}" autocomplete="off" class="form-horizontal">
            ...
            <div class="card ">
              <div class="card-header card-header-rose card-header-icon">
                ...
                <h4 class="card-title">{{ __('Izmjena podataka') }}</h4>
              </div>
              <div class="card-body ">
                <div class="row">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" id="input-name" type="text" placeholder="{{ __('') }}" value="{{ old('name', $user->name) }}" required="true" aria-required="true"/>
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('role_id') ? ' has-danger' : '' }}">
                      <select class="selectpicker col-sm-12 pl-0 pr-0" name="role_id" data-style="select-with-transition" title="" data-size="100">
                        <option value="1" @if (old('role_id', $user->role_id) == '1') selected="selected" @endif>{{ __('Admininstrator') }}</option>
                        <option value="2" @if (old('role_id', $user->role_id) == '2') selected="selected" @endif>{{ __('Tehnolog') }}</option>
                        <option value="3" @if (old('role_id', $user->role_id) == '3') selected="selected" @endif>{{ __('Operater') }}</option>
                      </select>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('password') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('password') ? ' is-invalid' : '' }}" input type="password" name="password" id="input-password" placeholder="{{ __('') }}" />
                      @include('alerts.feedback', ['field' => 'password'])
                    </div>
                  </div>
                </div>
                <div class="row">
                  <label class="col-sm-2 col-form-label" for="input-password-confirmation">{{ __('Potvrda lozinke') }}</label>
                  <div class="col-sm-7">
