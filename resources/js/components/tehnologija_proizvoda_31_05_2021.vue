...
<!-- PREGLED TEHNOLOGIJA -->
      <div class="tab-pane" id="link_pregled_tehnologija">
        ...
        <div class="row">
          Ovde treba biti lista naziva elemenata i njihovih tehnologija
        </div>  
      </div>
<!-- ANALIZA KAPACITETA -->
      <div class="tab-pane" id="link_analiza_kapaciteta">
        <div class="row">
          ...
          <div class="col-6 text-left">
                        <form  class="form-horizontal" @submit.prevent="snimiBrProizvoda()">
                          <div>
                  ...
                  <div class="card-body">                    
                        ...
                        <tr v-for="(k_materijal, index) in kalkulacija_materijali" v-bind:key="index">
                          ...
                          <td v-if="k_materijal.mattip_naziv===k_matTip.mattip_naziv" class="text-right">
                            {{zaokruzi(parseFloat(k_materijal.potrosnja)*k_materijal.cijena,2)}}
                            <div v-if="k_materijal.ima_kontakt!=='0'"><br>{{zaokruzi(parseFloat(k_materijal.potrosnja)*parseFloat(k_materijal.k_koef)*parseFloat(k_materijal.k_cijena),2)}}</div>
                            <div v-if="k_materijal.ima_razredjivac!=='0'"><br>{{zaokruzi(parseFloat(k_materijal.potrosnja)*parseFloat(k_materijal.r_koef)*parseFloat(k_materijal.r_cijena),2)}}</div>
                          </td>  
                        </tr>
                                  <tr>
  ...
  total_materijal () {
    return this.kalkulacija_materijali.reduce( (acc, k_materijal) => {
      return parseFloat(acc) 
           + parseFloat( this.zaokruzi( parseFloat(k_materijal.cijena) * parseFloat(k_materijal.potrosnja), 2))
           + parseFloat( this.zaokruzi( parseFloat(k_materijal.k_cijena) * parseFloat(k_materijal.potrosnja) * parseFloat(k_materijal.k_koef), 2))
           + parseFloat( this.zaokruzi( parseFloat(k_materijal.r_cijena) * parseFloat(k_materijal.potrosnja) * parseFloat(k_materijal.r_koef), 2))
...
//alert('6');        
  ...
  total_tipa (mt_naziv) {
    return this.kalkulacija_materijali.reduce( (acc, k_materijal) => {
      return parseFloat(acc) 
           + parseFloat((mt_naziv==k_materijal.mattip_naziv)?this.zaokruzi(parseFloat(k_materijal.cijena)   * parseFloat(k_materijal.potrosnja), 2):parseFloat(0))
           + parseFloat((mt_naziv==k_materijal.mattip_naziv)?this.zaokruzi(parseFloat(k_materijal.k_cijena) * parseFloat(k_materijal.potrosnja) * parseFloat(k_materijal.k_koef), 2):parseFloat(0))
           + parseFloat((mt_naziv==k_materijal.mattip_naziv)?this.zaokruzi(parseFloat(k_materijal.r_cijena) * parseFloat(k_materijal.potrosnja) * parseFloat(k_materijal.r_koef), 2):parseFloat(0))
    ...
    //  OPERACIJE  //////////////////////////////
        ucitajOperacije: function() {
          ...
          .then((response)=>{
              ...
              //kada osvjezavas prikaz operacija: - osvjezi i analizu 
              this.ucitajAnalizuKapaciteta();
              //                                  - osvježi kalkulaciju
              this.ucitajKalkulacijaMaterijali(); 
          })
...
// 
        ...
        show_PDF(){
         ...
         var page = 'http://localhost/fpdf183/tutorial/analiza_pdf.php?id='+document.getElementById("proizvod_id").value;
         //var page = '/test_pdf';
         
         var myWindow = window.open(page, "_blank", "scrollbars=yes,width=650,height=1000,top=300");
